...
and/or actions:
    ...
    exec    = command [arg ...]         # where {} is replaced by path
    print  [= print-opts]
and/or depth criteria:
    maxdepth = maximum depth to recurse in path
    mindepth = minimum depth to recurse before checking files or directories
...
class NameOption(Option):
    ...
    Option name is 'name', e.g. {'name': '*.txt'}
    '''
...
class InameOption(Option):
    '''
    Match files with a case-insensitve glob filename pattern.
...
class TypeOption(Option):
    '''
    Match files by the file type(s).
    ...
    separated list of letters.
        ...
        p = FIFO (named pipe)
        s = socket
    The option is 'type', e.g. {'type': 'd'} or {'type': 'bc'}.
    '''
    def __init__(self, key, value):
...
class OwnerOption(Option):
    ...
    A match occurs when the file's uid matches any userspecified.
...
class GroupOption(Option):
    '''
    Match files by their group name(s) and/or uid(s), e.g. 'root'.
    The names are a space and/or comma separated list of names and/or integers.
    A match occurs when the file's gid matches any group specified.
    The option name is 'group', e.g. {'group': 'root'}.
    '''
    def __init__(self, key, value):
        self.gids = set()
        for name in value.replace(',', ' ').split():
...
class GrepOption(Option):
    ...
    The option name is 'grep', e.g. {'grep': '(cool)|(story)|(bro)'}.
    '''
    def __init__(self, key, value):
        try:
            self.regex = re.compile(value)
...
class PrintOption(Option):
    ...
    one or more of the following:
        group = group name
        md5    = MD% digest of file contents
...
class ExecOption(Option):
    '''
    Execute the given command, {} replaced by the filename.
...
class Finder(object):
    def __init__(self, options):
        ...
        for key, value in six.iteritems(options):
            ...
            try:
                # Assign the class via the option in ``options`` e.g. 'iname' = InameOption
                # the result will be InameOption('iname', 'value') stored in ``obj``
        ...
        # Order criteria so least expensive checks are made first
